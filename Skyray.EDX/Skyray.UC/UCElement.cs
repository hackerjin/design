    ...
    public partial class UCElement : Skyray.Language.UCMultiple
    {
        ...
        private CurveElement AddElement(string Caption, int index, int line, IntensityWay IntensityWay, LayerFlag LayerFlag, int rowsIndex)
        {
            ...
            CurveElement curve = CurveElement.New.Init(Caption, true, Caption, AtomId, index, "", (XLine)line, 0, 0, 0, 0, false, atomDensity, 0, 0, 0.00, IntensityWay, false,
                                    ...
                                    ContentUnit.per, ThicknessUnit.um, "", "", "", "", 0, 0, 0, 0, 0, 0, 0, 0, 0, rowsIndex, "", false, color, " ", 0, "", false, true, false,
                                    false, 1, 0, "", "", "", false, Caption, false, 0, 0,false);
        ...
        private CurveElement SetDefaultPeak(CurveElement element)
        {
            ...
            if (atom != null)
            {
                ...
                if (Id <= 56)
                {
                    ...
                    //energyL2 = Atoms.AtomList.Find(a => a.AtomName == element.Caption).Kb;
                }
                else if (Id == 79 || Id == 73 || Id == 77 || Id == 78 || Id == 82)  //Au, Ta,Ir,Pt,Pb默认LB
                {
                    ...
                    //energyL2 = Atoms.AtomList.Find(a => a.AtomName == element.Caption).La;
                    //element.LbXLine = XLine.La;
                }
                else
                {
                    ...
                    //energyL2 = Atoms.AtomList.Find(a => a.AtomName == element.Caption).Lb;
                    //element.LbXLine = XLine.Lb;
                }

                energyL2 = Atoms.AtomList.Find(a => a.AtomName == element.Caption).Lb;
                int channel = DemarcateEnergyHelp.GetChannel(energy);
                int channelLb2 = DemarcateEnergyHelp.GetChannel(energyL2);
                ...
                element.LPeakLeft = channelLb2 - DifferenceDevice.PeakLowHightSpacing < 0 ? 0 : channelLb2 - DifferenceDevice.PeakLowHightSpacing;
                element.LPeakLeft = element.LPeakLeft > specLength ? specLength - 1 : element.LPeakLeft;
                element.LPeakRight = channelLb2 + DifferenceDevice.PeakLowHightSpacing > specLength ? specLength - 1 : channelLb2 + DifferenceDevice.PeakLowHightSpacing;
                element.LPeakRight = element.LPeakRight < 0 ? 0 : element.LPeakRight;
        ...
        private bool SaveElements()
        {
            ...
            string sqlelemlist = "Update ElementList Set IsRemoveBk =" + isRemoveBack +
              " Where Id = " + workCurveCurrent.ElementList.Id;
            Lephone.Data.DbEntry.Context.ExecuteNonQuery(sqlelemlist);
        ...
        private void dgvwElements_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (dgvwElements.Columns[e.ColumnIndex].Name.Equals("colIsDeduct"))//扣除元素
            {
                //if ((dgvwElements[dgvtxt.Name, e.RowIndex].Value != null) && (dgvwElements[dgvtxt.Name, e.RowIndex].Value.ToString() != Info.Substrate))
                //{
                if (dgvwElements[e.ColumnIndex, e.RowIndex].Value != null)
                {
                    if (dgvwElements[e.ColumnIndex, e.RowIndex].Value.Equals(false))
                    {
                        for (int i = 0; i < this.dgvwElements.RowCount; i++)
                        {
                            if (i == e.RowIndex)
                            {
                                workCurveCurrent.ElementList.Items[e.RowIndex].IsDeduct = true;
                            }
                            else
                            {
                                dgvwElements[e.ColumnIndex, i].Value = false;
                                workCurveCurrent.ElementList.Items[i].IsDeduct = false;
                            }
                        }
                    }
                    else
                    {
                        workCurveCurrent.ElementList.Items[e.RowIndex].IsDeduct = false;
                    }
                }
                else
                {
                ...
                //else
                //{
                //    dgvwElements[e.ColumnIndex, e.RowIndex].Value = false;
                //    workCurveCurrent.ElementList.Items[e.RowIndex].IsDeduct = false;
                //}
            }
        ...
        private void chkRemoveBack_Click(object sender, EventArgs e)
        {
            ...
            if (chkRemoveBack.Checked)
            {
                ...
                List<SpecListEntity> sample = EDXRFHelper.GetReturnSpectrum(false, false);
                if (sample != null && sample.Count > 0)
                {
                    workCurveCurrent.ElementList.SpecListName = sample[0].Name; //sample.specListCurrent.Name;
                    workCurveCurrent.ElementList.IsRemoveBk = true;
                }
                else
                {
            ...
            {
                workCurveCurrent.ElementList.SpecListName = null;
