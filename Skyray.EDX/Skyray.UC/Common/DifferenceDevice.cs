        ...
        public static void TransHeightPure(WorkCurve workCurve, double Hegiht, bool isRcalcSample, SpecListEntity templist, int setCurrent)
        {
            ...
            foreach (var element in workCurve.ElementList.Items)
            {
                ...
                if (isRcalcSample)
                {
                    ...
                    {
                        if (element.LayerNumBackUp == "基材" || element.LayerNumBackUp == "Substrate") continue;
            ...
            try
            {
                ...
                if (pureSampleName != "")//改变纯元素谱数据
                {
                    ...
                    */
                    ...
                    /*
                    if (Hegiht <= splitHeight)
                    {
                        for (int m = 0; m < pureList.Count; m++)
                        {
                            if ((float)pureList[m].Height <= splitHeight)
                            {

                                if (isAdjust)
                                    tempPoint = new PointF((float)pureList[m].Height, (float)Math.Round(pureList[m].CurrentUnifyCount, 6));

                                else
                                    tempPoint = new PointF((float)pureList[m].Height, (float)Math.Round(pureList[m].TotalCount, 6));
                        ...
                        CalSquareParam(lstActivePoint.Distinct().ToArray(), coeff, dim, false);
                    ...
                    */
                    ...
                    if (IsPureElemCurrentUnify)
                    {
                        ...
                        /*
                        lstActivePoint.Clear();
                        lstActivePoint.Add(new PointF(0f,1f));
                        if (Hegiht <= splitHeight)
                        {
                            for (int m = 0; m < pureList.Count; m++)
                            {
                                if ((float)pureList[m].Height <= splitHeight)
                                {
                                    ...
                                    if (isAdjust)
                                        tempPoint = new PointF((float)pureList[m].Height, (float)Math.Round(pureList[m].CurrentUnifyCount / ZeroTotalCount, 6));

                                    else
                                        tempPoint = new PointF((float)pureList[m].Height, (float)Math.Round(pureList[m].TotalCount / ZeroTotalCount, 6));
                            ...
                            CalSquareParam(lstActivePoint.Distinct().ToArray(), coeffLine, dim, false);
