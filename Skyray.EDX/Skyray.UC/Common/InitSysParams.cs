    ...
    public class InitSysParams
    {
        ...
        private static void LoadRohsParams(string path)
        {
            try
            {
                ...
                XmlNodeList xmlCalibrationList = doc.SelectNodes("Parameter/DataParam/NiCustomFiled");
                if (xmlCalibrationList != null)
                {
                    foreach (XmlNode xTemp in xmlCalibrationList)
                    {
                        if (xTemp.Name == "NiCustomFiled")
                        {
                            WorkCurveHelper.NiCuNiCustomParam.Limit = xTemp.Attributes["Limit"].InnerText == null ? 0.35 : double.Parse(xTemp.Attributes["Limit"].InnerText);
                            WorkCurveHelper.NiCuNiCustomParam.OptValue = xTemp.Attributes["OptValue"].InnerText == null ? 1 : double.Parse(xTemp.Attributes["OptValue"].InnerText);
                            WorkCurveHelper.NiCuNiCustomParam.CustomFiled1 = xTemp.Attributes["CustomFiled1"].InnerText == null ? "x" : xTemp.Attributes["CustomFiled1"].InnerText.ToLower();
                            WorkCurveHelper.NiCuNiCustomParam.CustomFiled2 = xTemp.Attributes["CustomFiled2"].InnerText == null ? "x" : xTemp.Attributes["CustomFiled2"].InnerText.ToLower();
                            break;
                        }
                ...
                {
                    WorkCurveHelper.NiCuNiCustomParam.Limit = 0.35;
                    WorkCurveHelper.NiCuNiCustomParam.CustomFiled1 = "x";
                    WorkCurveHelper.NiCuNiCustomParam.CustomFiled2 = "x";
                    WorkCurveHelper.NiCuNiCustomParam.OptValue = 1;
                }
        ...
        public static void LoadAppParamsInit(string path)
        {
            try
            {
                ...
                node = doc.SelectSingleNode("application/TabResetX");
                WorkCurveHelper.TabResetX = node == null ? 125 : float.Parse(node.InnerText);
                if (node == null)
                {
                    ReportTemplateHelper.SaveSpecifiedValue(path, doc, "application/TabResetX", new Dictionary<string, string>()
                    {
                        {
                            "TabResetX", "125"
                        }
                ...
                node = doc.SelectSingleNode("application/TabResetY");
                WorkCurveHelper.TabResetY = node == null ? 125 : float.Parse(node.InnerText);
                if (node == null)
                {
                    ReportTemplateHelper.SaveSpecifiedValue(path, doc, "application/TabResetY", new Dictionary<string, string>()
                    {
                        {
                            "TabResetY", "125"
                        }
                ...
                node = doc.SelectSingleNode("application/isAdjustSpecData");
                WorkCurveHelper.isAdjustSpecData = node == null ? false : bool.Parse(node.InnerText);
                if (node == null)
                {
                    ReportTemplateHelper.SaveSpecifiedValue(path, doc, "application/isAdjustSpecData", new Dictionary<string, string>()
                    {
                        {
                            "isAdjustSpecData", "false"
                        }
                    }, true);
                }

        
                node = doc.SelectSingleNode("application/fineGainAdjustParam");
                WorkCurveHelper.fineGainAdjustParam = node == null ? new RectangleF(0, 0, 0, 0) : new RectangleF(float.Parse(node.InnerText.Split(new char[] { ',' })[0]), float.Parse(node.InnerText.Split(new char[] { ',' })[1]), float.Parse(node.InnerText.Split(new char[] { ',' })[2]), 0);
                if (node == null)
                {
                    ReportTemplateHelper.SaveSpecifiedValue(path, doc, "application/fineGainAdjustParam", new Dictionary<string, string>()
                    {
                        {
                            "fineGainAdjustParam", "0,0,0"
                        }
                    }, true);
                }


                node = doc.SelectSingleNode("application/initCountRate");
                WorkCurveHelper.initCountRate = node == null ? 0 : float.Parse(node.InnerText);
                if (node == null)
                {
                    ReportTemplateHelper.SaveSpecifiedValue(path, doc, "application/initCountRate", new Dictionary<string, string>()
                    {
                        {
                            "initCountRate", "0"
                        }
