...
namespace Skyray.UC
{
    ...
    public partial class UcTabParams : Skyray.Language.UCMultiple
    {
        ...
        {
            if (numTabResetX.Value > WorkCurveHelper.DeviceCurrent.MotorXMaxStep)
            {
                Msg.Show("平台X复位点不可大于X轴最大行程");
                return;

            }
            if (numTabResetY.Value > WorkCurveHelper.DeviceCurrent.MotorYMaxStep)
            {
                Msg.Show("平台Y复位点不可大于Y轴最大行程");
                return;
            ...
            WorkCurveHelper.TabResetX = (float)numTabResetX.Value;

            Skyray.EDX.Common.ReportTemplateHelper.SaveSpecifiedParamsValue(AppDomain.CurrentDomain.BaseDirectory + "AppParams.xml", "application/TabResetX", WorkCurveHelper.TabResetX.ToString());

            WorkCurveHelper.TabResetY = (float)numTabResetY.Value;

            Skyray.EDX.Common.ReportTemplateHelper.SaveSpecifiedParamsValue(AppDomain.CurrentDomain.BaseDirectory + "AppParams.xml", "application/TabResetY", WorkCurveHelper.TabResetY.ToString());
        ...
        {
            numTabResetX.Value = (decimal)WorkCurveHelper.TabResetX;
            numTabResetY.Value = (decimal)WorkCurveHelper.TabResetY;
