...
namespace Skyray.UC
{
    ...
    public partial class UCHistoryRecord : Skyray.Language.UCMultiple
    {
        ...
        void setItem_Click(object sender, EventArgs e)
        {
            ...
            string historyItemPath = ReportTemplateHelper.GetHistoryItemPath(((MyItem)comboBoxCurveName.SelectedItem).Value.ToString(),//(MyItem)comboBoxCurveName.SelectedItem).Value.ToString(),
        ...
        private void UCHistoryRecord_Load(object sender, EventArgs e)
        {
            ...
            splitContainerHistory.SplitterDistance = WorkCurveHelper.HistoryTitileHight;
            if (!Lephone.Data.DbEntry.Context.GetTableNames().Contains("Condition"))
                Skyray.EDXRFLibrary.Condition.FindAll();


            string sql = "select * from WorkCurve a inner join Condition b on  a.Condition_Id = b.Id and a.IsShowMain inner join Device c on b.Device_Id=c.Id where c.Id=" + WorkCurveHelper.DeviceCurrent.Id;
            ...
            comboBoxCurveName.BeginUpdate();  // 暂停绘制
            try
            {
                ...
                {
                    if (!workcurve.IsShowMain) continue;
            ...
            {
                comboBoxCurveName.EndUpdate();  // 恢复绘制
            }
            ...
            if (WorkCurveHelper.IsDBOpen)
                this.btnExportSQL1.Visible = true;
            else
                this.btnExportSQL1.Visible = false;
        ...
        private void buttonWPrint_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewColumn column in this.dgvHistoryRecord.Columns)
            {
                if (column.Visible && column.Index > 0)
                {
                    ...
                    if ((FpWorkCurve.thickMode == ThickMode.NiNi || FpWorkCurve.thickMode == ThickMode.NiCuNi || FpWorkCurve.thickMode == ThickMode.NiCuNiFe || FpWorkCurve.thickMode == ThickMode.NewNiCuNi) && column.Name.Contains("Ni2"))
                    {
        ...
        private void buttonWExcel_Click(object sender, EventArgs e)
        {
            ...
            if (FpWorkCurve.thickMode == ThickMode.NiNi || FpWorkCurve.thickMode == ThickMode.NiCuNi || FpWorkCurve.thickMode == ThickMode.NiCuNiFe || FpWorkCurve.thickMode == ThickMode.NewNiCuNi)
            {

                string colName = WorkCurveHelper.WorkCurveCurrent.ElementList.Items.ToList().Find(w => w.LayerNumBackUp == Info.Substrate || w.LayerNumBackUp == "基材" || w.LayerNumBackUp == "Substrate").Caption;
                foreach (DataColumn dctemp in dt.Columns)
                {
                    // if (dctemp.ColumnName.Contains("Fe" + Info.Thick))
                    if (dctemp.ColumnName.Contains(colName + Info.Thick))
                    {
        ...
        private void OutExcel(bool bStatics, bool bExportReport)
        {
            ...
            {
                if (sheet.Cells[0, i].Value.ToString().Contains("含量") || sheet.Cells[0, i].Value.ToString().Contains("厚度") || sheet.Cells[0, i].Value.ToString().Contains("面密度"))
                {
        ...
        private void SetTestResult(ref DataTable dt)
        {
            ...
            bool isQuery = false;
            if (DifferenceDevice.IsRohs || DifferenceDevice.IsThick || WorkCurveHelper.isShowXRFStandard == 1 || countryId == 0)
            {
                isQuery = QueryHistoryElem(dt, ref HistoryElementlist);
            }
            if (DifferenceDevice.IsThick)
            {
                if (isQuery && HistoryElementlist.Count > 0)
                    foreach (DataRow row in dt.Rows)
                    {
                        List<HistoryElement> tempList = HistoryElementlist.Where(obj => obj.Id == Convert.ToInt64(row["HistoryRecord_Id"])).ToList();
                        string strCustomStandName = string.Empty;
                        // string strCustomStandName = string.Empty;
                        row["TestResult"] = ExcelTemplateParams.TestRetultForThick(tempList, out strCustomStandName);// DifferenceDevice.interClassMain.TestRetult(row["HistoryRecord_Id"].ToString()); //InterfaceClass.TestRetult(row["HistoryRecord_Id"].ToString());
        ...
        private bool QueryHistoryElem(DataTable dt, ref List<HistoryElement> HistoryElementlist)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    historyid += "'" + row["HistoryRecord_Id"].ToString() + "',";
                }
                historyid = dt.Rows.Count > 0 ? historyid.Substring(0, historyid.Length - 1) : historyid;
                string sql = " select HistoryRecord_Id,elementName,contextelementValue,unitValue,Error,customstandard_Id,thickelementValue from HistoryElement where HistoryRecord_Id in (" + historyid + ")";

                using (SQLiteConnection conn = new SQLiteConnection(Lephone.Data.DbEntry.Context.Driver.ConnectionString))
                {
                    conn.Open();
                    using (SQLiteCommand cmd = new SQLiteCommand(sql, conn))
                    {
                        ...
                        HistoryElement helem;
                        while (reader.Read())
                        {
                            helem = HistoryElement.New;
                            helem.Id = (long)reader["HistoryRecord_Id"];
                            helem.elementName = (string)reader["elementName"];
                            helem.contextelementValue = (string)reader["contextelementValue"];
                            helem.unitValue = (int)reader["unitValue"];
                            helem.Error = (double)reader["Error"];
                            helem.customstandard_Id = (long)reader["customstandard_Id"];
                            helem.thickelementValue = (string)reader["thickelementValue"];
                            HistoryElementlist.Add(helem);
                        }
        ...
        private string GetSQL(int sType, int totalPage, int currentPage, string strSelectID)
        {
            ...
            {
                //  workcurve_id = "-1";
                if (WorkCurveHelper.WorkCurveCurrent == null) workcurve_id = "-1";
                else
                    workcurve_id = WorkCurveHelper.WorkCurveCurrent.Id.ToString();
            }
            else
            {
                if (((MyItem)comboBoxCurveName.SelectedItem).Type == ItemType.Curve)
            ...
            strSql += " left outer join Atom e on e.atomname=elementName " +
                ...
                // " select id from elementlist where 1=1 " + ((comboBoxCurveName.Text == "--All--") ? " and workcurve_id in (select id from workcurve where condition_id in ( " +
                      " select id from elementlist where 1=1 " + ((workcurve_id == "-1") ? " and workcurve_id in (select id from workcurve where condition_id in ( " +
      " select id from condition where device_id=" + WorkCurveHelper.DeviceCurrent.Id + "))" : " and workcurve_id in " + workcurve_id) + " ) ) c on " + ("c.caption=elementName") +//氧化物排序问题
            ...
            if (DifferenceDevice.IsThick || countryId == 0 || WorkCurveHelper.isShowXRFStandard == 1) strSql += " '' as TestResult,'' as CustomStandard, ";
            ...
            string strIntellCondition = " type<>" + ((int)ConditionType.Intelligent).ToString() + " and ";
        ...
        private void GetSepecifiedSearchConditionRefresh(int totalPage, int currentPage, Skyray.Controls.DataGridViewW dgvHistory)
        {
            ...
            for (int i = 0; i < dgvHistory.Columns.Count; i++)
            {
                if (dgvHistory.Columns[i].HeaderText == "编号")
                {
                    dgvHistory.Columns[i].SortMode = DataGridViewColumnSortMode.NotSortable;

                    for (int j = 0; j < dgvHistory.Rows.Count; j++)
                    {


                        dgvHistory.Rows[j].Cells[i].Value = currentPage * ROWS_PER_PAGE + j + 1;
        ...
        private void btStatisticShow_Click(object sender, EventArgs e)
        {
            ...
            {
                if (this.sheetTemp.Cells[0, i].Value.ToString().Contains("含量") || this.sheetTemp.Cells[0, i].Value.ToString().Contains("厚度"))
                {
                ...
                for (int index = 0; index < workCurveCurrent.ElementList.Items.Count; index++)
                {
                    if (workCurveCurrent.ElementList.Items[index].IsShowElement)
                    {
                        ...
                        string sqlUp = "update CurveElement set ThicknessUnit = " + "2" + " where Caption = '" + workCurveCurrent.ElementList.Items[index].Caption + "'" +
                            ...
                            " and  ElementList_Id = " + workCurveCurrent.ElementList.Id;
        ...
        private void chkSelectWithRows_CheckedChanged(object sender, EventArgs e)
        {
            if (this.chkSelectWithRows.Checked)
            {
                ...
                bool flag1 = int.TryParse(this.txtBeginRow.Text, out beginRow);
                bool flag2 = int.TryParse(this.txtEndRow.Text, out endRow);

                if (!flag1 || !flag2 || beginRow < 1 || endRow < 1)
                {            
                    return;
                }
                ...
                foreach (DataRow his in dt.Rows)
                    temp.Add((long)his["Id"]);
                temp.Reverse();
                ...
                foreach (DataGridViewRow row in dgvHistoryRecord.Rows)
                {
                    ...
                    long selectId = long.Parse(row.Cells["Id"].Value.ToString());
                    if (selectLong.Contains(selectId))
                        row.Cells["aa"].Value = true;
                    else
                        row.Cells["aa"].Value = false;
                }
            ...
            dgvHistoryRecord.Refresh();
           
        }

        private void lblTotaleRowCount_Click(object sender, EventArgs e)
        {

        }

        private void txtBeginRow_TextChanged(object sender, EventArgs e)
        {
            if (this.chkSelectWithRows.Checked)
            {
        ...
        private void splitContainerHistory_SplitterMoved(object sender, SplitterEventArgs e)
        {
            ...
            //XmlHelper.SetContainerValue(doc, "UCHistoryRecord", "splitContainerHistory", splitContainerHistory);
            //XmlHelper.RecordContainerValue(doc, "UCHistoryRecord", "splitContainerHistory", splitContainerHistory);
