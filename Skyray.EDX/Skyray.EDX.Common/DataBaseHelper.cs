            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                switch (row["tbl_name"].ToString())
                {
                    ...
                    case "DeviceParameter":
                        ...
                        if (row["sql"].ToString().IndexOf("Thickness") == -1) sql += " alter table DeviceParameter add Thickness float default 1.9;";
                        if (row["sql"].ToString().IndexOf("FaceTubCurrent") == -1) sql += " alter table DeviceParameter add FaceTubCurrent int default 0; update DeviceParameter set FaceTubCurrent = TubCurrent;";
                    ...
                    case "CurveElement":
                        ...
                        if (row["sql"].ToString().IndexOf("IsActiveLb") == -1) sql += "alter table CurveElement add IsActiveLb bool default 0; ";
                        if (row["sql"].ToString().IndexOf("LPeakLeft") == -1) sql += "alter table CurveElement add LPeakLeft int default 0;";
                        if (row["sql"].ToString().IndexOf("LPeakRight") == -1) sql += "alter table CurveElement add LPeakRight int default 0;";
                        if (row["sql"].ToString().IndexOf("IsDeduct") == -1) sql += "alter table CurveElement add IsDeduct bool default 0;";
