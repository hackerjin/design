...
namespace Skyray.Thick
{
    public partial class FrmThick : MultipleForm, IRefreshFillInfo
    {
        ...
        public float countRate()
        {
           ...
           return  (float)DifferenceDevice.interClassMain.deviceMeasure.interfacce.ReturnCountRate;
        }
        ...
        public void ContructMeasureRefreshData(int count, ElementList elementList)
        {
            ...
            foreach (var elementlQuery in query)
            {
                ...
                {   //厚度
                    ...
                    string baseName = WorkCurveHelper.WorkCurveCurrent.ElementList.Items.ToList().Find(w => w.LayerNumBackUp == Info.Substrate || w.LayerNumBackUp == "基材" || w.LayerNumBackUp == "Substrate").Caption;
                   
                    if ((FpWorkCurve.thickMode == ThickMode.NiNi || FpWorkCurve.thickMode == ThickMode.NiCuNi || FpWorkCurve.thickMode == ThickMode.NiCuNiFe ||
                        FpWorkCurve.thickMode == ThickMode.NiCuNiFe2 || FpWorkCurve.thickMode == ThickMode.NewNiCuNi)
                        && elemColumns.Name.ToUpper().Equals(baseName.ToUpper()))
                        elemColumns.HeaderText = "Ni2" + "(" + headName + ")";
        ...
        public void CreateContructStatis(ElementList elementList)
        {
            ...
            foreach (var elementlQuery in query)
            {
                ...
                else
                {
                    ...
                    if ((FpWorkCurve.thickMode == ThickMode.NiNi || FpWorkCurve.thickMode == ThickMode.NiCuNi || FpWorkCurve.thickMode == ThickMode.NiCuNiFe
                        || FpWorkCurve.thickMode == ThickMode.NiCuNiFe2) && caption.Substring(0, caption.Length - 1).ToUpper().Equals(baseelemName.ToUpper()))
        ...
        public void FrmThick_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            try
            {
                ...
                if (e.CloseReason == CloseReason.UserClosing)
                {
                    ...
                    XmlHelper.RecordContainerValue(doc, "UCHistoryRecord", "splitContainerHistory", ucHistoryRecord1.splitContainerHistory);
        ...
        private void LoadUI()
        {
            try
            {
                ...
                XmlHelper.SetContainerValue(doc, this.Name, "UCHistoryRecord", ucHistoryRecord1.splitContainerHistory);
                WorkCurveHelper.HistoryTitileHight = ucHistoryRecord1.splitContainerHistory.SplitterDistance;
